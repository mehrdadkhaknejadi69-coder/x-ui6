...
REAL_TRANSPORT_PROTOCOLS = {
    ...
    "vmess-ws", "trojan-ws", "highspeed-demo",
}
...
# این‌ها فقط لینک ساخته می‌شن ولی هیچ سرویس‌دهنده‌ی واقعی روی سرور ندارن (باقی‌مانده از
# نسخه‌ی قبلی پروژه) — برای شفافیت به کاربر پنل هم در پاسخ /api/protocols علامت زده می‌شن.
NON_FUNCTIONAL_DEMO_PROTOCOLS = {"shadowsocks", "socks5", "http", "hysteria2", "tuic", "wireguard", "gaming-lite-demo"}
...
PROTOCOL_LABELS = {
    ...
    "shadowsocks": "Shadowsocks (غیرفعال — دمو)",
    "socks5": "SOCKS5 (غیرفعال — دمو)",
    "http": "HTTP Proxy (غیرفعال — دمو)",
    "hysteria2": "Hysteria 2 (غیرفعال — دمو)",
    "tuic": "TUIC (غیرفعال — دمو)",
    "wireguard": "WireGuard (غیرفعال — دمو)",
...
def verify_admin_credentials(username: str | None, password: str):
    ...
    if not username or username.lower() == "owner":
        if hash_password(password) == AUTH["password_hash"]:
            return True, "owner", "owner", "مالک پنل"
...
def get_link_info(
    ...
    return {
        ...
        "network": normalize_network(link.get("network")),
        "security": normalize_security(link.get("security")),
        "manual_live": (normalize_network(link.get("network")), normalize_security(link.get("security"))) in MANUAL_LIVE_COMBOS,
...
/* VodiWalker 13.0.1 responsive system */
...
<div class="desc">
این صفحه، درگاه عمومی VodiWalker است.
...
<a href="/plans" class="btn secondary">خرید اشتراک</a>

<a
href="https://t.me/VodiWalker"
target="_blank"
rel="noopener"
class="btn secondary"
>
...
async def api_protocols(request: Request, _=Depends(require_auth)):
    return {
        "protocols": [
            {
                ...
                "functional": p in REAL_TRANSPORT_PROTOCOLS or p in REAL_RAW_TCP_PROTOCOLS,
            }
            for p in PROTOCOLS
        ],
        ...
        "manual": {
            ...
            "live_combos": [list(c) for c in MANUAL_LIVE_COMBOS],
        },
    }
...
# ============================================================
# GRANT VOLUME / DAYS (شارژ دستی توسط ادمین)
# ============================================================

@app.post(
    "/api/links/{uid}/grant"
)
async def grant_link_volume(
    uid: str,
    request: Request,
    _=Depends(require_auth),
):
    ...
    add_gb = safe_float(body.get("volume_gb", 0))
    add_days = safe_int(body.get("days", 0), minimum=0)

    if add_gb <= 0 and add_days <= 0:
        raise HTTPException(
            status_code=400,
            detail="مقدار حجم یا تعداد روز را وارد کنید",
        )
    ...
    async with LINKS_LOCK:
        ...
        was_unlimited = int(link.get("limit_bytes") or 0) <= 0

        if add_gb > 0 and not was_unlimited:
            add_bytes = int(add_gb * (1024 ** 3))
            link["limit_bytes"] = int(link.get("limit_bytes") or 0) + add_bytes

        if add_days > 0:
            base = datetime.now()
            existing = link.get("expires_at")
            if existing:
                try:
                    ...
                    pass
            link["expires_at"] = (
                base + timedelta(days=add_days)
            ).isoformat()

        label = link.get("label", uid)
        new_limit = link.get("limit_bytes")
        new_expires = link.get("expires_at")
    ...
    if add_gb > 0:
        parts.append(
            f"{add_gb:g} گیگ" + (" (کانفیگ نامحدود بود، بدون تغییر)" if was_unlimited else "")
        )
    if add_days > 0:
        parts.append(f"{add_days} روز")

    log_activity(
        "link",
        f"شارژ دستی برای «{label}»: " + " و ".join(parts),
        "info",
    )
...
async def subscription_portal(uuid: str, request: Request):
    ...
    label = escape_html(str(link.get("label") or "VodiWalker Subscription"))
    protocol = escape_html(protocol_display_label(link))
    used = int(link.get("used_bytes", 0) or 0)
    limit = int(link.get("limit_bytes", 0) or 0)
    if limit > 0:
        pct = min(100, round((used / limit) * 100, 1))
        traffic = f"{fmt_bytes(used)} / {fmt_bytes(limit)}"
        remaining = fmt_bytes(max(0, limit-used))
    else:
        pct = 0
        traffic = f"{fmt_bytes(used)} / نامحدود"
        remaining = "نامحدود"

    expires = link.get("expires_at") or "نامحدود"
    ip_limit = int(link.get("ip_limit", 0) or 0)
    cfg_count = max(1, min(40, int(link.get("config_count") or 1)))
    safe_raw = escape_html(raw_url)
    safe_info = escape_html(info_url)
    qr_data = quote(raw_url, safe="")

    html = f"""<!doctype html>
<html lang="fa" dir="rtl"><head>
<meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">
<meta name="theme-color" content="#080b14"><title>{label} — VodiWalker</title>
<style>
*{{box-sizing:border-box}}body{{margin:0;background:#070a12;color:#eef2ff;font-family:Tahoma,Arial,sans-serif}}
body:before{{content:"";position:fixed;inset:-20%;z-index:-1;background:radial-gradient(circle at 20% 10%,rgba(99,102,241,.25),transparent 28%),radial-gradient(circle at 85% 20%,rgba(6,182,212,.18),transparent 25%)}}
.wrap{{width:min(980px,92%);margin:auto;padding:35px 0 60px}}.top{{display:flex;justify-content:space-between;gap:16px;align-items:center;margin-bottom:22px}}
.brand{{font-weight:900;font-size:20px}}.status{{padding:8px 12px;border:1px solid #24304a;border-radius:999px;background:#101729;color:#7dd3fc;font-size:12px}}
.hero,.card{{background:rgba(15,23,42,.78);border:1px solid rgba(255,255,255,.08);border-radius:28px;backdrop-filter:blur(18px);box-shadow:0 25px 90px rgba(0,0,0,.28)}}
.hero{{padding:30px;margin-bottom:18px}}h1{{font-size:clamp(26px,5vw,46px);margin:8px 0}}.muted{{color:#94a3b8;line-height:1.9}}.grid{{display:grid;grid-template-columns:1fr 1fr;gap:18px}}.card{{padding:24px}}
.stat{{display:flex;justify-content:space-between;gap:10px;padding:13px 0;border-bottom:1px solid rgba(255,255,255,.06)}}.stat:last-child{{border:0}}.bar{{height:10px;background:#182238;border-radius:99px;overflow:hidden;margin:15px 0}}.bar i{{display:block;height:100%;width:{pct}%;background:linear-gradient(90deg,#6366f1,#06b6d4);border-radius:99px}}
.url{{direction:ltr;text-align:left;word-break:break-all;padding:14px;border-radius:15px;background:#080d18;border:1px solid #1d2940;color:#c7d2fe;font-size:12px;margin:12px 0}}
button,a.btn{{border:0;cursor:pointer;text-decoration:none;display:inline-flex;align-items:center;justify-content:center;padding:13px 16px;border-radius:15px;font-weight:800;color:white;background:linear-gradient(135deg,#6366f1,#06b6d4);margin:4px}}
a.alt{{background:#121b2d;border:1px solid #263451}}.actions{{display:flex;flex-wrap:wrap;margin-top:8px}}
.qr{{display:flex;justify-content:center;margin:8px 0 18px}}.qr img{{width:190px;height:190px;background:white;padding:10px;border-radius:20px}}
.small{{font-size:12px;color:#71809b}}@media(max-width:760px){{.grid{{grid-template-columns:1fr}}.top{{align-items:flex-start}}}}
</style></head><body><main class="wrap">
<div class="top"><div class="brand">⚡ VodiWalker</div><div class="status">● اشتراک فعال</div></div>
<section class="hero"><div class="muted">Subscription Portal</div><h1>{label}</h1><div class="muted">مدیریت اشتراک، کپی لینک و بررسی وضعیت در یک صفحه. لینک خام برای کلاینت‌ها جدا نگه داشته شده تا سازگاری نسخه‌های قبلی حفظ شود.</div></section>
<section class="grid">
<div class="card"><h2>📊 وضعیت اشتراک</h2><div class="stat"><span>پروتکل</span><b>{protocol}</b></div><div class="stat"><span>ترافیک</span><b>{traffic}</b></div><div class="bar"><i></i></div><div class="stat"><span>باقی‌مانده</span><b>{remaining}</b></div><div class="stat"><span>انقضا</span><b>{escape_html(str(expires)[:19])}</b></div><div class="stat"><span>محدودیت IP</span><b>{ip_limit or '∞'}</b></div><div class="stat"><span>تعداد کانفیگ</span><b>{cfg_count}</b></div></div>
<div class="card"><h2>🔗 لینک اشتراک</h2><div class="qr"><img src="https://api.qrserver.com/v1/create-qr-code/?size=220x220&data={qr_data}" alt="QR"></div><div class="url" id="suburl">{safe_raw}</div><div class="actions"><button onclick="copySub()">📋 کپی لینک</button><a class="btn alt" href="{safe_raw}">⬇ دریافت اشتراک</a><a class="btn alt" href="{safe_info}">ℹ جزئیات</a></div><p class="small">این لینک را داخل کلاینت سازگار با VLESS/Subscription وارد کنید.</p></div>
</section></main>
<script>async function copySub(){{try{{await navigator.clipboard.writeText(document.getElementById('suburl').innerText);alert('لینک کپی شد ✅')}}catch(e){{prompt('کپی کنید:',document.getElementById('suburl').innerText)}}}}</script>
</body></html>"""
...
async def info_page(
    ...
    label_escaped = escape_html(snapshot.get("label", "VodiWalker"))
...
<div class="w-full max-w-4xl mx-auto space-y-5 sm:space-y-6 md:space-y-8">
    ...
    <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-5">
      <div class="flex items-center gap-4">
        ...
        <div class="min-w-0">
          ...
          <p class="mt-1.5 text-[10.5px] sm:text-[11px] text-white/40 break-all">UUID: {uid_escaped} &nbsp;·&nbsp; VodiWalker {app_version_str}</p>
        </div>
      </div>
  ...
  <div class="rounded-2xl border border-emerald-400/15 bg-emerald-400/[0.05] p-4 text-center text-xs text-white/45">
    پشتیبانی و اطلاعیه‌ها &nbsp;·&nbsp; <b class="text-emerald-300">کانال تلگرام: VodiWalker</b>
  </div>
